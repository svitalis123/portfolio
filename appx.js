...
for (let b = 0; b < projects.length; b++){
    const buttona = document.querySelector(`#${projects[b]}`);
    buttona?.addEventListener('click', () => {
    const divDetails = document?.querySelector(`#${projects[b]}amend1`);
    ...
    style="height:220px; width:311px; position: relative; "
     alt="project image" />;
    <div id=${projects[b]}  class="closexs" style="position:absolute; top:1px; right: 5px;"><i class="fa fa-close" style="float:right; padding:12px; color:white; font-size:xx-large;" ></i></div>
...
closebtn.addEventListener('click', () => {
  ...
  for(let a in divDetails){
  divDetails[a].classList?.remove('detailsssx');
  }
}) 
}
...
for (let c = 0; c < projects.length; c++){
  const buttonab = document.querySelector(`#${projects[c]}`);
  buttonab?.addEventListener('click', () => {
  const divDetails2 = document.querySelector(`#${projects[c]}amend1desc`);
  ...
  <div id="${projects[c]}dis" class="closexl" style="position:absolute; top:2%; right: 5%;"><i class="fa fa-close" style="float:right; padding:12px; color:#000; font-size:xx-large;" ></i></div>
  ...
  `;
  challdesk(`${projects[c]}dis`);
});

}

const challdesk = (str) => {
    var closebtn2 = document.getElementById(`${str}`);
    closebtn2.addEventListener('click', () => {
      var divDetails2 = document.querySelectorAll('[id*="amend1desc"]');
      for (let d in divDetails2){
        divDetails2[d].classList.remove('details_ammenddescreverse');
      }
    })
}
